...
import jp.vmi.selenium.selenese.command.EndBlock;
import jp.vmi.selenium.selenese.command.ICommand;
import jp.vmi.selenium.selenese.command.ICommandFactory;
import jp.vmi.selenium.selenese.command.StartBlock;
...
import static jp.vmi.selenium.selenese.command.StartBlock.*;
...
public class TestCase implements Selenese, ITestCase, IHtmlResultTestCase {
    ...
    private StartBlock currentStartBlock = NO_START_BLOCK;
    private final Deque<StartBlock> blockStack = new ArrayDeque<>();
    ...
    public void addCommand(ICommand command) {
        command.setStartBlock(currentStartBlock);
        if (command instanceof EndBlock) {
            currentStartBlock.setEndBlock((EndBlock) command);
            currentStartBlock = blockStack.pop();
        }
        if (command instanceof StartBlock) {
            blockStack.push(currentStartBlock);
            currentStartBlock = (StartBlock) command;
        }
